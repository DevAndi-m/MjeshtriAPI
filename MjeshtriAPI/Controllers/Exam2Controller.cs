...
namespace MjeshtriAPI.Controllers
{
    ...
    public class Exam2Controller : ControllerBase
    {
        private readonly ApplicationDbContext _context;

        public Exam2Controller(ApplicationDbContext context)
        {
            _context = context;
        }

        [HttpPost("create-planet")]
        public async Task<IActionResult> CreatePlanet(CreatePlanetDTO2 planet)
        {
        ...
        public async Task<IActionResult> CreateSatellite(CreateSatelliteDTO2 satellite)
        {
            Satellite2 s = new Satellite2
            {
                Name = satellite.Name,
                PlanetId = satellite.PlanetId
            };

            _context.Satellites2.Add(s);
            await _context.SaveChangesAsync();
        ...
        {
            var planets = await _context.Planets2.ToListAsync();
        ...
        {
            var satellites = await _context.Satellites2
                .Where(s => s.IsDeleted == false)
                .Include(s => s.Planet)
                .Select(s => new GetSatelliteDTO2
                {
        ...
        public async Task<IActionResult> UpdateSatellite(UpdateSatelliteDTO2 sat)
        {
            var satellite = await _context.Satellites2.FirstOrDefaultAsync(s => s.SatelliteId == sat.SatelliteId);
        ...
        {
            var satellite = await _context.Satellites2.FirstOrDefaultAsync(s => s.SatelliteId == id);
            if (satellite == null)
            {
                return NotFound(new
                {
